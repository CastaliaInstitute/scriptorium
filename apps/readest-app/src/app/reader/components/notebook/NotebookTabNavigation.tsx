...
import { PiBookOpenText, PiNotePencil, PiRobot } from 'react-icons/pi';
...
const NotebookTabNavigation: React.FC<NotebookTabNavigationProps> = ({
  ...
  const tabs: NotebookTab[] = aiEnabled ? ['notes', 'codex', 'ai'] : ['notes', 'codex'];
  ...
  const getTabIcon = (tab: NotebookTab) => {
    switch (tab) {
      ...
      case 'codex':
        return <PiBookOpenText className='mx-auto' size={20} />;
      case 'ai':
